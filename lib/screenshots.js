...
module.exports = () => {
  ...
  const isValidScreenshotName = (nameSplit) => {
    ...
    if (
      nameSplit.length === 4 &&
      ['secondary', 'tertiary', 'quaternary', 'quinary'].indexOf(
        nameSplit[1]
      ) === -1
    ) {
      console.log(
  ...
  const formatScreenshot = (screenshot) => {
    ...
    const isQuaternary = nameSplit[1] === 'quaternary'
    const isQuinary = nameSplit[1] === 'quinary'
    ...
    const result = {
      state,
      url: `https://covidtracking.com/screenshots/${state}/${fileName}`,
